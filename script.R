...
run_region_analysis <- function(dataset_name, file_path, bins = 25, top_percent = 0.1) {
 ...
 cor_table_faceted <- df_long_lncRNA %>%
    dplyr::filter(!is.na(Shape_Value), !is.na(.data[[score_col]])) %>%
    dplyr::group_by(lncRNA, Feature) %>%
    dplyr::summarise(
      n = dplyr::n(),
      r = if (n >= 3) stats::cor(Shape_Value, .data[[score_col]], method = "pearson") else NA_real_,
      p_value = if (n >= 3) stats::cor.test(Shape_Value, .data[[score_col]], method = "pearson")$p.value else NA_real_,
      .groups = "drop"
    ) %>%
