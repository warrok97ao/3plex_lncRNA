...
median_diff <- function (path, neg_type){
  ...
  # Filter out the average columns for heatmap only
  cols_to_exclude <- c("Total_average", "TPX-validated_lncRNAs_average")
  median_matrix_plot <- median_matrix[, !colnames(median_matrix) %in% cols_to_exclude]
  pvalue_matrix_plot <- pvalue_matrix[, !colnames(pvalue_matrix) %in% cols_to_exclude]

  col_fun1 = circlize::colorRamp2(c(-1, 1), c("#f4a582", "#0571b0"))
  Heatmap(pvalue_matrix_plot, name = "Median_difference", col = col_fun1, rect_gp = gpar(type = "none"),
    column_order = c("7SK", "Bloodlinc", "1200007C13Rik@4T1", "AC018781.1", "1200007C13Rik@NDL", "AL109615.3", "MIR503HG", "MEG3", "CDKN2B-AS1", "HOTAIR", "lncSmad7", "TERC", "Rn7sk", "MALAT1", "NEAT1", "AC087482.1"),
    cell_fun = function(j, i, x, y, width, height, fill) {
      grid.rect(x = x, y = y, width = width, height = height, 
            gp = gpar(col = "grey", fill = NA))
        grid.circle(x = x, y = y, r = (-log10(pvalue_matrix_plot[i, j]))/9.5 * min(unit.c(width, height)), 
                gp = gpar(fill = col_fun1(median_matrix_plot[i, j]), col = NA))
...
path = "/home/molinerislab/RNABSdb_3DDNA/dataset/v1/ca_neg_regions/specific-no_low_DNase/ALL_shape.aggregated.fixed.gz"

pdf("Median_diff.heatmap.pdf", width = 12, height = 5, pointsize = 13)
median_diff("/home/molinerislab/RNABSdb_3DDNA/dataset/v1/ca_neg_regions/specific-no_low_DNase/ALL_shape.aggregated.fixed.gz", "all_cCREs")
dev.off()
```